...
std::shared_ptr<HornclauseNode> Parser::ParseHornclause(){
    ...
    if (*scanner_.Peek() == TokenType::LEFTPAREN &&
        *scanner_.Peek(1) == TokenType::LEFTPAREN) // multi-predicate head
        hornclause.AddNode(ParseMultiPredHead());
    else
        hornclause.AddNode(ParseSinglePredHead()); // parse head.
    if (*scanner_.Peek() == TokenType::LEFTPAREN){
...
std::shared_ptr<HeadNode> Parser::ParseSinglePredHead(){
    HeadNode head;
    head.AddNode(ParsePredicate()); // parse predicate
    return std::make_shared<HeadNode>(head);
}

std::shared_ptr<HeadNode> Parser::ParseMultiPredHead() {
    HeadNode head;
    match(TokenType::LEFTPAREN);
    do {
        head.AddNode(ParsePredicate());
    } while (*scanner_.Peek() == TokenType::LEFTPAREN);
    match(TokenType::RIGHTPAREN);
    return std::make_shared<HeadNode>(head);
} 
